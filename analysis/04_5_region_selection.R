...
df_binned <- df |>
  ...
  filter(fused < 0.05, fpa = 1) |>

  # Filter to retain only data that is in bins 1, 3, and 5 for MI and Lat.
  # and for bins 1 and 3 for relief
  filter(
    ai_bin %in% c(...),  # retain only data that is in bins 1, 3, and 5
    relief_bin %in% c(...),  # retain only data that is in bins 1 and 3
    lat_bin %in% c(... )  # retain only data that is in bins 1, 3, and 5
  )
