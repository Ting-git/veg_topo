...
# ----------- boxplot and violin together -------------------------

# Define fill colors
fill_colors <- RColorBrewer::brewer.pal(8, "Set2")

# Generate plots for comparing different clusters
pbox_cor    <- plot_box_or_violin(df, "cluster8c", "cor",    "boxplot", "Correlation")
pvio_cor    <- plot_box_or_violin(df, "cluster8c", "cor",    "violin",  NULL)
pbox_ai     <- plot_box_or_violin(df, "cluster8c", "ai",     "boxplot", "AI")
pvio_ai     <- plot_box_or_violin(df, "cluster8c", "ai",     "violin",  NULL)
pbox_fused  <- plot_box_or_violin(df, "cluster8c", "fused",  "boxplot", "Fused")
pvio_fused  <- plot_box_or_violin(df, "cluster8c", "fused",  "violin",  NULL)
