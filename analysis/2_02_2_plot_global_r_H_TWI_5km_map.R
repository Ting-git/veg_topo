...
# 1.2 Read TWI-vegetation height correlation raster
r_H_TWI <- rast(cor_twi_vegh_mosaic_file)

# 1.3 Convert to data frame and clean
r_H_TWI_clean <- as.data.frame(r_H_TWI, xy = FALSE) |>
  drop_na()
colnames(r_H_TWI_clean)[1] <- "correlation"

# 1.4 Create grouping variable (positive / negative correlation)
r_H_TWI_clean <- r_H_TWI_clean |>
...
percent_positive <- sum(r_H_TWI_clean$correlation > 0) / nrow(r_H_TWI_clean) * 100
percent_negative <- sum(r_H_TWI_clean$correlation < 0) / nrow(r_H_TWI_clean) * 100
...
# =============================================================================
p1 <- ggplot(r_H_TWI_clean, aes(x = correlation, fill = group)) +
...
r_H_TWI <- aggregate(r_H_TWI, fact = c(2, 2))

# 3.2 Build map
p_cor <- ggplot2::ggplot() +
  # Spatial raster layer
  tidyterra::geom_spatraster(data = r_H_TWI, maxcell = Inf) +
