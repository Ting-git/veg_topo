...
#   - R/plot_hex_scatter.R, R/plot_r_H_R.R (custom plotting functions)
# ==============================================================================
...
p_used <- plot_r_H_R(
  input = cor_used,
  extent = ext_global,
  title = "Pearson’s r (H～Rᵢₙ) on used land (fused ≥ 0.05)",
  text_size = 12,
  x_step = 30, y_step = 30
) +
...
p_natural <- plot_r_H_R(
  input = cor_natural,
  extent = ext_global,
  title = "Pearson’s r (H～Rᵢₙ) on natural land (fused < 0.05)",
  text_size = 12,
  x_step = 30, y_step = 30
) +
  guides(fill = guide_colorbar(
...
df <- as.data.frame(c(cor_r, fused_5km_r, mi_5km_r), xy = TRUE, na.rm = TRUE)
...
final_plot <-
  ...
  plot_annotation(
    title = "Land use influence on Pearson's r (H～Rᵢₙ)",
