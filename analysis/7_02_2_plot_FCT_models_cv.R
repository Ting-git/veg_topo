...
importance_combined <- readRDS(file.path(rf_models_dir, "importance_combined.rds"))

# 从 combined importance 中提取各模型数据
importance_full <- importance_combined %>% filter(Model == "Full")
importance_climate <- importance_combined %>% filter(Model == "Climate Only")
importance_topo <- importance_combined %>% filter(Model == "Topography Only")
...
r2_values <- model_comparison %>%
  ...
  select(Model, R2 = CV_R2, RMSE = CV_RMSE)
...
ggsave(
  here::here("data/figures/7_02_model_r2_horizontal_with_importance_cv.png"),
  p_horizontal,
...
cat("\n✅ Done! Saved to: data/figures/7_02_model_r2_horizontal_with_importance_cv.png\n")
